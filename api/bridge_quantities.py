...
from models import BridgeQuantities
...
router = APIRouter(prefix="/bridge_quantities", tags=["定量描述管理"])


@router.get("/", summary="分页查询定量描述列表")
async def get_bridge_quantities_list(
    page: int = Query(1, ge=1, description="页码"),
    size: int = Query(20, ge=1, le=100, description="每页数量"),
    name: Optional[str] = Query(None, description="定量描述名称模糊查询"),
    session: Session = Depends(get_db),
):
    """分页查询定量描述列表"""
    service = get_base_crud_service(BridgeQuantities, session)
...
@router.post("/", summary="创建定量描述")
async def create_bridge_quantities(
...
@router.put("/{id}", summary="更新定量描述")
async def update_bridge_quantities(
    ...
    if not item:
        raise NotFoundException(resource="BridgeQuantities", identifier=str(id))

    response_item = Response.model_validate(item)
    return success(response_item.model_dump(), "更新成功")


@router.delete("/{id}", summary="删除定量描述")
async def delete_bridge_quantities(id: int, session: Session = Depends(get_db)):
    """删除定量描述"""
    service = get_base_crud_service(BridgeQuantities, session)
