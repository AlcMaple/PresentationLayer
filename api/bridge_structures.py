...
from models import BridgeStructures
...
router = APIRouter(prefix="/bridge_structures", tags=["结构类型管理"])


@router.get("", summary="分页查询结构类型列表")
async def get_bridge_structures_list(
    page: int = Query(1, ge=1, description="页码"),
    size: int = Query(20, ge=1, le=100, description="每页数量"),
    name: Optional[str] = Query(None, description="结构类型名称模糊查询"),
    session: Session = Depends(get_db),
):
    """分页查询结构类型列表"""
    service = get_base_crud_service(BridgeStructures, session)
...
@router.post("", summary="创建结构类型")
async def create_bridge_structures(
...
@router.put("/{id}", summary="更新结构类型")
async def update_bridge_structures(
    ...
    if not item:
        raise NotFoundException(resource="BridgeStructures", identifier=str(id))

    response_item = Response.model_validate(item)
    return success(response_item.model_dump(), "更新成功")


# @router.delete("/{id}", summary="删除结构类型")
# async def delete_bridge_structures(id: int, session: Session = Depends(get_db)):
#     """删除结构类型"""
#     service = get_base_crud_service(BridgeStructures, session)
